...
| `DELETE` | `/api/goals/{id}` | Delete a goal and its contributions |
| `POST` | `/api/goals/{id}/contributions` | Record money in (or out, as a negative); returns the whole goal |
| `DELETE` | `/api/goals/{id}/contributions/{contributionId}` | Undo a recorded contribution |
...
A goal is a number and, optionally, a date. Everything else is worked out.

Contributions are **rows, not a running total**. The old `goals.current_amount`
column is gone, replaced by a `goal_contributions` ledger summed on read by
`goal_saved()`. One figure cannot tell you whether someone is saving steadily or
put everything in on day one and has added nothing since, and those two people
need to be told completely different things. It also means the total can never
drift from the deposits that produced it.

Money can come **back out**, stored as a negative amount rather than a separate
kind, so the total stays a plain sum. Raiding your own savings is a thing that
happens, and a goal that could not show it would be lying.

The pace is measured **from the first contribution, not from when the goal was
created**. Someone who set a goal in January and started saving in March has
been saving for two months; dividing by five would tell them they are doing
worse than they are.

Most of the design is about **refusing to answer**. `null` is used freely and
means "we cannot say":

- **No target date, no verdict.** There is no required monthly amount and the
  goal can never be "behind" — there is nothing to be behind. It still gets
  progress and a projected finish date.
- **Too new to judge.** Under two weeks of history, no pace is reported. One
  deposit either flatters someone enormously or makes a brand-new goal look
  abandoned.
- **Nothing going in projects to nothing**, not to a date. The interface says so
  in words rather than showing a fabricated finish date.
- **A passed deadline is reported as passed**, before anything about monthly
  amounts — there is no longer a figure that arrives "in time". This ordering is
  pinned by a test, because every one of these states is true at once and the
  wrong branch produces a sentence that is accurate and completely misleading.

Reaching the target moves the goal to `achieved` automatically and stamps
`achieved_at`, which is **never cleared**. A later withdrawal drops the progress
bar below 100%, but hitting the goal was a thing that happened on a particular
day and spending some of it afterwards does not undo the day. Paused and
cancelled goals are left alone — a status the user chose is not ours to
overwrite.

Progress is **not** read from the linked account. An account holds money for
many reasons, and treating its balance as goal progress would credit someone for
a salary that landed yesterday.
...
- Balances are **derived** through `account_balance()`, never incremented per
  ...
  Goal progress follows the same rule through `goal_saved()`.
